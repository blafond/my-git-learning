...
public class ReactiveCollectionUpdateAction extends CollectionAction implements ReactiveExecutable {
	...
	public CompletionStage<Void> reactiveExecute() {
		...
		// FIXME: This kind of info is better if added to the JavaDoc of the class. It makes it easier to figure out
		//       which class is used as reference in ORM.
		//       Also, not critical, but `TODO` and `FIXME` are recognized as special comments by intellij, `FYI` isn't
		// FYI:  The content of this method was copied from CollectionUpdateAction in hibernate-orm
		// ... and modified to fix compile errors and started replacing persister calls to Reactive persister

		preUpdate();

		//TODO: To make this work in reactive you need to make sure that the operations are executed in the right order
...
//		else {
//			// TODO: I don't know exactly what this does, but I'm assuming it should call the proper reactive methods in
//			//       reactivePersister and be in updateStage
//			updateStage = updateStage
...
//		}
		...
		// TODO: All this should be in updateStage.thenAccept(..) or it will be executed even if the previous operations
		//       haven't finished yet and it should be the last thing to do before returning
